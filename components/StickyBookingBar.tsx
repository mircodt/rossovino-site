...
export function StickyBookingBar({ property }: Props) {
  ...
  // Tutti gli header property della prova di design sono alti 96px (h-24)
  // e non hanno la vecchia strip "← Tutte le strutture", quindi la barra
  // sticky si posiziona sempre a 96px.
  const topClass = "top-[96px]";
  ...
  const bar = (
    <div
      ...
      // La barra è portalata su <body>, fuori dal wrapper a tema della
      // pagina: il tema della struttura va riapplicato qui.
      style={property ? propertyTheme(property) : undefined}
      aria-hidden={!show}
    >
