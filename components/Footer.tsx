  ...
  return (
    <footer className="bg-vinaccia-dark text-white mt-24 pb-24 lg:pb-12">
      <div className="mx-auto w-full max-w-[1200px] px-5 md:px-8 py-14">
        ...
        {/* Le tre strutture. Su mobile: solo testi in orizzontale (3 colonne,
            niente immagini) così il cliente percepisce subito 3 sedi, come sul
            desktop. Da sm in su: immagine + contatti completi. */}
        <div className="grid grid-cols-3 gap-x-3 gap-y-0 sm:gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {PROPERTY_ORDER.map((slug) => {
            const p = PROPERTIES[slug];
            return (
              <div key={slug} className="text-center sm:text-left">
                <Link href={`/${slug}`} className="group block">
                  <div className="hidden sm:block relative aspect-[16/10] overflow-hidden rounded-[3px] mb-4">
                    <Image
                  ...
                  <div className="font-display text-[15px] sm:text-lg leading-tight mb-1 sm:mb-3 group-hover:text-sabbia transition-colors">
                    {p.fullName}
                  </div>
                </Link>
                {/* Città — visibile solo su mobile per aiutare la percezione "3 sedi" */}
                <p className="sm:hidden text-xs text-white/65 mb-2">
                  {p.address.addressLocality} ({p.address.addressRegion})
                </p>
                <ul className="space-y-1.5 sm:space-y-2 text-[11px] sm:text-sm text-white/85">
                  {hasContact(p.phone) && (
                    <li>
                      <a
                  ...
                  <li className="hidden sm:list-item text-white/80">
